...
class AttendanceDetailTest extends TestCase
{
     ...
     */
    public function test_logged_in_user_name_is_displayed()
    {
     ...
     */
    public function test_start_and_end_time_match()
    {
        $response = $this->get('/attendance/detail/' . $this->attendance->id);
        $response->assertStatus(200);

        // inputのvalue属性に出勤・退勤時刻が設定されていること
     ...
     */
    public function test_all_breaks_displayed_with_empty_row()
    {
        $response = $this->get('/attendance/detail/' . $this->attendance->id);
        $response->assertStatus(200);
    ...
    public function test_approved_apply_displayed_in_approved_tab()
    {
        ...
        // 承認済みタブ：表示されること
        preg_match('/<div class="list approve">(.*?)<\/div>/s', $waitContent, $approveMatches);
